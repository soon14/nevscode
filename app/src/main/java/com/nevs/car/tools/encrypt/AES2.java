 ...
 *
 * 编码方式的改变，加密的时候使用DES/AES加密后需要使用Base64编码方式再进行一次加密，解密的时候需要先用Base64解密，再用DES/AES相应的方法解密
 */
public class AES2 {
    // 算法名称
    ...
    public byte[] decrypt(byte[] encryptedData, byte[] keyBytes) {
        byte[] encryptedText = null;
        byte[] encryptedDataBase64=Base64.decode(new String(encryptedData));
        init(keyBytes);
        System.out.println("IV：" + new String(iv));
        MLog.e("a");
        try {
            ...
            MLog.e("b");
            encryptedText = cipher.doFinal(encryptedDataBase64);
            MLog.e("c");
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
