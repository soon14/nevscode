...
public class FingerprintUtil {
    ...
    public static  void callFingerPrint(final OnCallBackListenr listener){
        FingerprintManagerCompat managerCompat = FingerprintManagerCompat.from(MyApp.getInstance());
        if (!managerCompat.isHardwareDetected()){ //判断设备是否支持
            if (listener != null)
                listener.onSupportFailed();
            return;
        }
        KeyguardManager keyguardManager =(KeyguardManager)MyApp.getInstance().getSystemService(MyApp.getInstance().KEYGUARD_SERVICE);
