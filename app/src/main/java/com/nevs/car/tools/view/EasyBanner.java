...
import com.nevs.car.tools.util.DensityUtil;
...
public class EasyBanner extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    private void initData() {
        ...
        for (int i = 0; i < bannerSize; i++) {
            ...
            // CC imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
            //让外部去实现图片加载，实现解耦
            if (imageLoader != null) {
            ...
            final View finalPointView = pointView;
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mPointLayout.addView(finalPointView);  //添加到linearLayout中
                }
            });
            //CC    mPointLayout.addView(pointView);  //添加到linearLayout中
        ...
        //CC     mViewPager.setAdapter(bannerAdapter);
        //页面切换监听器
...
//                                try {
//                                    container.addView(imageView);  //cc
//                    }catch (Exception e){
//                        MLog.e("EasyBanner异常");
//                    }
