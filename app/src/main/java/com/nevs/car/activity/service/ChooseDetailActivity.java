...
public class ChooseDetailActivity extends BaseActivity implements AMapLocationListener {
    ...
    private void initIntent() {
        citynames = getIntent().getStringExtra("cityname");
        MLog.e(citynames + "cityname");
        if (citynames != null) {
            cityname.setText(getIntent().getStringExtra("citynames"));
            getService(citynames);
        } else {
            initLoacation();//定位
        }
...
//                            xRefreshView1.stopLoadMore(false);
    ...
    private void getService(String s) {
        ...
        HttpRxUtils.getDealerList(ChooseDetailActivity.this,
                new String[]{"accessToken", "type", "pid", "cid"},
                new Object[]{new SharedPHelper(ChooseDetailActivity.this).get(Constant.ACCESSTOKEN, ""),
                        "dealer", "", s
                },
                new HttpRxListener() {
                    @Override
                    public void onSucc(Object obj) {
                        DialogUtils.hidding(ChooseDetailActivity.this);
                        ...
                        if (listPoal.size() == 0) {
                            ...
                            ActivityUtil.showToast(ChooseDetailActivity.this, getResources().getString(R.string.toast_nopro));
                            initLoacation();
                        } else {
    ...
    private void initOnclickListener() {
        ...
        myAdapter.setOnRecyclerViewItemClickListener(new BaseQuickAdapter.OnRecyclerViewItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                finishSelect(listPoal.get(position).get("name").toString(), listPoal.get(position).get("orgcode").toString(), listPoal.get(position).get("type") + "");
            }
        });
        //条目长按点击事件
    ...
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            case R.id.back:
                ...
                break;
            case R.id.cityname:
                startActivityForResult(new Intent(this, SelectCityActivity.class), 805);
                break;
            case R.id.refresh:
                getService(cityname.getText().toString());
                break;
