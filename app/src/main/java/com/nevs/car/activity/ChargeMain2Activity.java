...
import com.nevs.car.activity.gmap.PoiChangeActivity;
import com.nevs.car.adapter.BaseQuickAdapter;
import com.nevs.car.adapter.ChargeMainAdapter;
...
public class ChargeMain2Activity extends BaseActivity implements LocationSource, AMapLocationListener
    ...
    private double lon;
    private double lat;
    private boolean idfresh = false;//是否加载成功
    List<HashMap<String, Object>> listTsp = new ArrayList<>();
    private double latme;
    private double lonme;
    ...
    public int getContentViewResId() {
        return R.layout.activity_charge_main2;
    }
    ...
    public void init(Bundle savedInstanceState) {
        ...
        try {
            makepoint2(new LatLng(latme, lonme), 1);
        } catch (Exception e) {
            MLog.e("makepoint 异常");
        }
    ...
    private void initIntent() {
        if (getIntent().getSerializableExtra("search") != null) {
            List<AddressBean> list = (List<AddressBean>) getIntent().getSerializableExtra("search");
            int i = getIntent().getIntExtra("possion", 0);
            latme = list.get(i).getLatitude();
            lonme = list.get(i).getLongitude();
    ...
    private void getList() {
        ...
        listTsp.clear();
        TspRxUtils.getStation(this,
                new String[]{Constant.TSP.CONTENTTYPE, Constant.TSP.ACCEPT, Constant.TSP.AUTHORIZATION},
                new Object[]{Constant.TSP.CONTENTTYPEVALUE, Constant.TSP.ACCEPTVALUE, "Bearer" + " " + new SharedPHelper(ChargeMain2Activity.this).get(Constant.ACCESSTOKENS, "")},
                latme + "", lonme + "", "20",
                new TspRxListener() {
                    @Override
                    public void onSucc(Object obj) {
                        MyUtils.upLogTSO(mContext, "充电列表", String.valueOf(obj), MyUtils.getTimeNow(), MyUtils.getTimeNow(), "", MyUtils.timeStampNow() + "");

                        isClick = true;
                        DialogUtils.hidding((Activity) mContext);
                        listTsp.addAll((Collection<? extends HashMap<String, Object>>) obj);
                        if (listTsp.size() == 0) {
...
//
//                            } else {
//                                ActivityUtil.showToast(mContext, getResources().getString(R.string.toast_unservicestop));
//                            }
                    ...
                    public void onFial(String str) {
                        ...
                        if (str.contains("400") || str.contains("无效的请求")) {
                            ActivityUtil.showToast(mContext, getResources().getString(R.string.unsreen));
                        ...
                        isClick = false;
                        MyUtils.upLogTSO(mContext, "充电列表", String.valueOf(str), MyUtils.getTimeNow(), MyUtils.getTimeNow(), "", MyUtils.timeStampNow() + "");
    ...
    //根据地址绘制需要显示的点
    public void makepoint2(final LatLng latLng, int mipmap) {
        Log.e("tag", "开始绘图");
        //自定义点标记
        ...
        if (mipmap == 1) {
            markerOptions.icon(BitmapDescriptorFactory.fromBitmap(BitmapFactory
                    .decodeResource(getResources(), R.mipmap.dh_wz)));//设置图标csxt1
        } else {
            markerOptions.icon(BitmapDescriptorFactory.fromBitmap(BitmapFactory
                    .decodeResource(getResources(), R.mipmap.dh_wz)));//设置图标csxt1
        }
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                cameraUpdate = CameraUpdateFactory.newCameraPosition(new CameraPosition(latLng, 10.4f, 0, 30));
    ...
    private void getListss() {
        DialogUtils.loading(mContext, true);
        clears();
        TspRxUtils.getStationSreen(this,
                new String[]{Constant.TSP.CONTENTTYPE, Constant.TSP.ACCEPT, Constant.TSP.AUTHORIZATION},
                new Object[]{Constant.TSP.CONTENTTYPEVALUE, Constant.TSP.ACCEPTVALUE, "Bearer" + " " + new SharedPHelper(ChargeMain2Activity.this).get(Constant.ACCESSTOKENS, "")},
                latme + "", lonme + "", "9999", urls,
                new TspRxListener() {
                    @Override
                    public void onSucc(Object obj) {
                        isClick = true;
    ...
    private void clears() {
        ...
        clearMarks(list);
        clearMarks(list2);
        clearMarks(listchang);
        clearMarks(markerList);
        list.clear();
        list2.clear();
        listchang.clear();
        markerList.clear();
    ...
    private void clearMarks(List<Marker> ll) {
    ...
    private void initOnclickListener() {
        ...
        myAdapter.setOnRecyclerViewItemClickListener(new BaseQuickAdapter.OnRecyclerViewItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                //改变可视区域为指定位置
                //CameraPosition4个参数分别为位置，缩放级别，目标可视区域倾斜度，可视区域指向方向（正北逆时针算起，0-360）
                cameraUpdate = CameraUpdateFactory.newCameraPosition(new CameraPosition(new LatLng(Double.parseDouble(listTsp.get(position).get("latitude") + ""),
                        Double.parseDouble(listTsp.get(position).get("longitude") + "")), 13, 0, 30));
                aMap.moveCamera(cameraUpdate);//地图移向指定区域

                //将地图移动到显示MARK位置
                aMap.moveCamera(CameraUpdateFactory.changeLatLng(new LatLng(Double.parseDouble(listTsp.get(position).get("latitude") + ""),
                        Double.parseDouble(listTsp.get(position).get("longitude") + ""))
                ));

                name = listTsp.get(position).get("stationName").toString();
                text = listTsp.get(position).get("electricityFee") + "";
                dis = listTsp.get(position).get("distance").toString();
                lon = Double.parseDouble(listTsp.get(position).get("latitude") + "");
                lat = Double.parseDouble(listTsp.get(position).get("longitude") + "");
    ...
    private void initRecyclyView() {
        ...
        myAdapter = new ChargeMainAdapter(R.layout.rel_chargeguide, listTsp); //设置适配器
    ...
    private void moveTo() {
        aMap.moveCamera(CameraUpdateFactory.changeLatLng(latLngd));
    }
    ...
    public void onLocationChanged(AMapLocation aMapLocation) {
        if (aMapLocation != null) {
            if (aMapLocation.getErrorCode() == 0) {
                ...
                if (isFirstLoc) {
                    //设置缩放级别
                    aMap.moveCamera(CameraUpdateFactory.zoomTo(10.4f));
                    //将地图移动到定位点
                    ...
                    latLngd = new LatLng(aMapLocation.getLatitude(),
                            aMapLocation.getLongitude());//取出经纬度
//                    latme = aMapLocation.getLatitude();
//                    lonme = aMapLocation.getLongitude();
...
//                        strokeWidth(15));
    ...
    @OnClick({R.id.start_local, R.id.btn_book, R.id.poi_close, R.id.btn_guides, R.id.backs,
            R.id.imageone, R.id.rel_all, R.id.btn_alls})
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_guides:
                ...
                break;
            case R.id.imageone:
                startActivityForResult(new Intent(this, ScreeningActivity.class), 8001);
                break;
            case R.id.rel_all:
                startActivity(new Intent(this, PoiChangeActivity.class).putExtra("chargemainintent", "1"));
                break;
            case R.id.btn_alls:
                Intent intent = new Intent(this, ChargeEnterActivity.class);
                Bundle bundle = new Bundle();
                bundle.putSerializable("listss", (Serializable) listTsp);
                intent.putExtras(bundle);
                intent.putExtra("possionss", possionss);
                ...
                switch (position) {
                    ...
                    case 0:
                        ...
                        if (GaodeNaiUtils.isInstallPackage()) {
                            ...
                            ActivityUtil.showToast(ChargeMain2Activity.this, getResources().getString(R.string.toast_installgao));
                        }
                        break;
                    case 1:
                        //百度地图
                        if (BaiduNaiUtils.isInstalled()) {
                            ...
                            ActivityUtil.showToast(ChargeMain2Activity.this, "请先安装百度地图客户端");
                        }
                        break;
                }
                if (dialog != null) {
...
//        poiRel.setVisibility(View.GONE);
    ...
    private void drawbleMarks() {
        if (listTsp.size() != 0) {
            for (int i = 0; i < listTsp.size(); i++) {
                getAddress(Double.parseDouble(listTsp.get(i).get("latitude") + ""), Double.parseDouble(listTsp.get(i).get("longitude") + ""),
                        listTsp.get(i).get("stationName").toString(), listTsp.get(i).get("distance").toString(), ""
                );
            }
        }
    ...
    //根据地址绘制需要显示的点
    public void makepoint(String s, final double lats, final double lons, final String title, String distants, String texts) {
        Log.e("tag", "开始绘图");
        //北纬39.22，东经116.39，为负则表示相反方向
        LatLng latLngg = new LatLng(lats, lons);
        Log.e("tag", "地址:" + s);
        ...
        //自定义点标记
        MarkerOptions markerOptions = new MarkerOptions();
        markerOptions.position(new LatLng(lats, lons)).title(title).snippet(texts);
        markerOptions.icon(BitmapDescriptorFactory.fromBitmap(BitmapFactory
...
//        //改变可视区域为指定位置  MACK点击移到中间位置
//        //CameraPosition4个参数分别为位置，缩放级别，目标可视区域倾斜度，可视区域指向方向（正北逆时针算起，0-360）
//        cameraUpdate = CameraUpdateFactory.newCameraPosition(new CameraPosition(latLngg, 13, 0, 30));
//        aMap.moveCamera(cameraUpdate);//地图移向指定区域
//
//        //将地图移动到显示车辆位置
//        aMap.moveCamera(CameraUpdateFactory.changeLatLng(latLngg));
...
//                }
                ...
                if (marker.getPosition().longitude == latLngd.longitude || marker.getPosition().longitude == new LatLng(latme, lonme).longitude) {
                    MLog.e("判断MARK的点击事件");
                } else {
...
//                        lastMark = marker;
                        ...
                        for (int i = 0; i < listTsp.size(); i++) {
                            if ((listTsp.get(i).get("stationName") + "").equals(marker.getTitle())) {
                                name = listTsp.get(i).get("stationName") + "";
                                text = listTsp.get(i).get("electricityFee") + "";
                                dis = listTsp.get(i).get("distance") + "";
                                lon = Double.parseDouble(listTsp.get(i).get("latitude") + "");
                                lat = Double.parseDouble(listTsp.get(i).get("longitude") + "");
...
//                        list.get(i-1).setAlpha(0);
    ...
    private void getMark(Marker marker) {
        ...
        for (int i = 0; i < listTsp.size(); i++) {
            if (Double.parseDouble(listTsp.get(i).get("latitude") + "") == lat) {
                possionss = i;
    ...
    private void initBottomState() {
        ...
        try {
            poiCharge.setText(MyUtils.getDisOnePoint(text.split(":")[4]));
        } catch (Exception E) {
            MLog.e("电费异常");
            poiCharge.setText("");
        }

        poiDistance.setText(MyUtils.getDisTwoPoint(dis));
    }
    ...
    public String getDiatance(double lat, double lon) {//计算距离
        ...
        try {
            dis = String.valueOf(AMapUtils.calculateLineDistance(latLngd, new LatLng(lat, lon)));
            MLog.e("距离：M" + dis);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 8001 && resultCode == 8002) {
            urls = "";
            String parkFee = data.getStringExtra("parkFee");
            boolean equipmentIsFree = data.getBooleanExtra("equipmentIsFree", false);
            int stationType = data.getIntExtra("stationType", 5);
            String payment = data.getStringExtra("payment");
            MLog.e("parkFee:" + parkFee + "  equipmentIsFre:" + equipmentIsFree + "  stationType:" + stationType + "  payment:" + payment);
            if (parkFee.length() > 0) {
                urls += ("&parkFee=" + parkFee);
            }
            if (equipmentIsFree) {
                urls += ("&equipmentIsFree=" + equipmentIsFree);
            } else {
                urls += ("&equipmentIsFree=" + equipmentIsFree);
            }
            if (stationType == 5) {
                ...
                urls += ("&stationType=" + stationType);
            }
            if (payment.length() > 1) {
                urls += ("&payment=" + payment);
            }
            MLog.e("筛选latme" + latme + "  urls:" + urls);
            getListss();
        }
    }
